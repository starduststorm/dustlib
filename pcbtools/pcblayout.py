...
def findFootprintLibsFolder():
  ...
  if _footprintsLibFolder is None:
    path = Path(pcb.pcbnew_file).parent
...
class Point(object):
  ...
  def __init__(self, arg1, arg2=None):
    if arg2 is not None:
      self.x = arg1
      self.y = arg2
    elif pcb.is_wxpoint(arg1):
      p = Point.fromWxPoint(arg1)
      self.x = p.x
      self.y = p.y
    elif pcb.is_vector2i(arg1):
  ...
  def vector2i(self):
    return pcb.vector2i_mm(self.x, self.y)
...
def pretty(obj):
  ...
  elif type(obj) is pcb.PCB_VIA:
    return "<pcbnew.PCB_VIA ({}) at {} drill {} width {} [this={}]".format(obj.GetNetname(), Point(obj.GetPosition()), obj.GetDrill()/pcb.IU_PER_MM, obj.GetWidth()/pcb.IU_PER_MM, obj.this)
  elif type(obj) is pcb.FOOTPRINT:
    return "<pcbnew.FOOTPRINT '{}' at {}>", obj.GetReference(), Point(obj.GetPosition())
  elif type(obj) is pcb.BOARD_ITEM_CONTAINER:
    return str(obj)
  elif obj is None:
    return "pretty(None)"
  else:
...
class HashableItem:
  ...
  def __hash__(self):
    ...
    elif type(self.item) == pcb.PCB_TRACK:
    ...
    elif type(self.item) == pcb.PCB_ARC:
    ...
    elif type(self.item) == pcb.PCB_VIA:
...
def isTrack(t):
  return type(t) is pcb.PCB_TRACK or (type(t) is HashableItem and type(t.item) is pcb.PCB_TRACK)

def isPad(t):
  return type(t) is pcb.PAD or (type(t) is HashableItem and type(t.item) is pcb.PAD)

def isVia(t):
  return type(t) is pcb.PCB_VIA or (type(t) is HashableItem and type(t.item) is pcb.PCB_VIA)
...
class KiCadPCB(object):
  def __init__(self, path, lock=False):
    ...
    self.lock = lock
    self.board = pcb.LoadBoard(self.pcb_path)
    self.resetCache()
  ...
  def resetCache(self):
    ...
    self.numlayers = pcb.LAYER_ID_COUNT
  ...
  def drawVias(self, layer):
    ...
    for v in vias:
      ...
      width = v.GetWidth()/pcb.IU_PER_MM
      print(pretty(v))
  ...
  def itemLayersIntersect(self, item1, item2):
    ...
    if parent.GetFlags() & pcb.FLAG_DELETED:
  ...
  def draw_circle(self, center, radius, layer='F.Silkscreen', width=0.15, filled=False):
      ...
      circle = pcb.PCB_SHAPE()
      self.board.Add(circle)
      circle.SetShape(pcb.SHAPE_CIRCLE)
      circle.SetCenter(center.vector2i())
      circle.SetArcAngleAndEnd(pcb.angle_degrees(360))
  ...
  def draw_arc(self, start, center=None, end=None, angle=None, layer='F.Silkscreen', width=0.15):
      ...
      arc = pcb.PCB_SHAPE(self.board)
      arc.SetStart(start.vector2i())
      arc.SetShape(pcb.SHAPE_ARC)
  ...
  def add_via(self, position, net, fromLayer="F.Cu", toLayer="B.Cu"):
    ...
    if fromLayer != "F.Cu" or toLayer != "B.Cu":
      via.SetViaType(pcb.VIA_BLIND_BURIED)
...
class PCBLayout(object):
  ...
  def __init__(self, path, pixelType, footprintsPath, args=None):
    self.args = args
    self.kicadpcb = KiCadPCB(path, lock=args.lock if args else False)
    self.board = self.kicadpcb.board
    self.pixelType = pixelType
    self.footprintsPath = footprintsPath
  ...
  def addZonePoint(self, point, zoneName, netName, layers, priority=0):
    if zoneName in self.zones:
      ...
      zone.SetLocked(self.args.lock if self.args else False)
      ...
      if netName not in self.kicadpcb.netnames:
        net = pcb.NETINFO_ITEM(self.board, netName)
  ...
  def connectPixels(self, fromPixel, toPixel, connectorFunction=None):
    ...
    if self.args and not self.args.skip_traces:
  ...
  def insertFootprint(self, footprintNameOrPrototype, reference, point, orientation, layer):
    ...
    if type(footprintNameOrPrototype) is str:
      fp = pcb.FootprintLoad(self.footprintsPath, footprintNameOrPrototype)
      if not fp:
        lib, name = footprintNameOrPrototype.split(":")
        fp = pcb.FootprintLoad(lib, name)
      assert fp, "Failed to load footprint {}".format(footprintNameOrPrototype)
    else:
      fp = pcb.FOOTPRINT(footprintNameOrPrototype)
    ...
    fp.SetOrientation(pcb.angle_radians(orientation))
    fp.SetReference(reference)
    self.kicadpcb.board.Add(fp)
    fp.Reference().SetVisible(False)
    fp.SetLocked(self.args.lock if self.args else False)
    if onBack:
  ...
  def placePixel(self, reference, point, orientation, layer="F.Cu", allowOverlaps=True, alignOverlaps=True, powerConnect=None, groundConnect=None):
    ...
    if self.pixel_prototype is None:
      print("Loading pixel footprint at folder %s name %s" % (self.footprintsPath, self.pixelType.footprintName))
      self.pixel_prototype = pcb.FootprintLoad(self.footprintsPath, self.pixelType.footprintName)
      ...
      for fp in self.kicadpcb.board.GetFootprints():
        ...
        if absPoint.distance_to(fpPoint) < overlapThresh:
          if alignOverlaps:
            ...
            fp.SetOrientation(pcb.angle_radians(newOrientation))
  ...
  def rotateEach(self, pattern, angle):
    ...
    for fp in self.board.GetFootprints():
      if pat.match(fp.GetReference()):
        ...
        fp.SetOrientation(pcb.angle_degrees(orientation+angle))
        changed = True
    if not changed:
